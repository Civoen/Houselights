...
export default function LineupPage() {
  ...
  async function handlePosterUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // Searched in small concurrent batches, not all at once — a big
      // festival poster can easily have 80-100+ acts, and firing that many
      // simultaneous Spotify search requests in one burst is exactly the
      // kind of spike that can trip a sustained rate limit (especially
      // testing the same poster repeatedly), which used to be able to hang
      // the whole app rather than just this one feature.
      const BATCH_SIZE = 6;
      const matched: PosterMatch[] = [];
      for (let i = 0; i < names.length; i += BATCH_SIZE) {
        const batch = names.slice(i, i + BATCH_SIZE);
        const results = await Promise.all(
          batch.map(async (name): Promise<PosterMatch> => {
            try {
