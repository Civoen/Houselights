...
async function spotifyFetch(path: string, accessToken: string, init: RequestInit = {}, attempt = 0): Promise<any> {
  ...
  if (res.status === 429) {
    // Capped at 3 retries — uncapped retrying here previously meant a
    // sustained rate-limit condition (not just a brief burst) hung the
    // request forever: the browser's fetch to our own API route never
    // gets a response at all, showing as permanently "Pending" in
    // DevTools, with no error for the frontend's try/catch to ever
    // catch — indistinguishable from a frozen UI. A real error after a
    // bounded number of attempts is far better than an infinite hang.
    if (attempt >= 3) {
      throw new Error("Spotify is rate-limiting requests right now — try again in a moment.");
    }
    const retryAfter = Number(res.headers.get("Retry-After") || "1");
    await new Promise((r) => setTimeout(r, retryAfter * 1000));
    return spotifyFetch(path, accessToken, init, attempt + 1);
  }
  if (!res.ok) {
